...
public class SM4 {
    ...
    private byte[] padding(byte[] input, int mode) {
        ...
        if (mode == SM4_ENCRYPT) {
            ...
            String inputHex = EncryptionUtil.byteToHex(input)+ "80";
            StringBuffer stringBuffer =new StringBuffer(inputHex);
            for (int i = 0; i <p-1 ; i++) {
                stringBuffer.append("00");
            }
            ret= EncryptionUtil.hexToByte(stringBuffer.toString());
            ...
            String inputHex = EncryptionUtil.byteToHex(input);
            int i = inputHex.lastIndexOf("80");
            String substring = inputHex.substring(0, i);
            ret= EncryptionUtil.hexToByte(substring);
        }
        return ret;
    }
